...
class CheckoutVC: UIViewController,WKNavigationDelegate {
    ...
    func replaceShippingAddress(){
        ...
        self.theWebView.evaluateJavaScript("var wholeAddress = document.getElementsByClassName('shipping-info')[0].getElementsByTagName('span')[0];wholeAddress.childNodes[0].innerHTML =wholeAddress.childNodes[0].innerHTML.replace(wholeAddress.childNodes[0].innerHTML, '');wholeAddress.innerHTML=wholeAddress.innerHTML.replace('United States of America','');var lastChar = wholeAddress.innerHTML.substr(wholeAddress.innerHTML.length - 13);var findString = '';for (var x = 0; x < lastChar.length; x++){ var c = lastChar.charAt(x);if(c <='9' && c >='0' || c== '-') {findString+=c;}};if(findString.length >=5){wholeAddress.innerHTML=wholeAddress.innerHTML.replace(findString,'');wholeAddress.innerHTML = wholeAddress.innerHTML.replace('<br>',',');wholeAddress.innerHTML = wholeAddress.innerHTML.replace('<br>',',');};var array = wholeAddress.innerHTML.split(',');wholeAddress.innerHTML=wholeAddress.innerHTML.replace(array[0],'');wholeAddress.innerHTML=wholeAddress.innerHTML.replace(array[2],'');wholeAddress.innerHTML=wholeAddress.innerHTML.replace(',','');wholeAddress.innerHTML=wholeAddress.innerHTML.replace(',','');", completionHandler: { (object, error) -> Void in
            println(object)
        })
    }
